...
)
{
	...
	while (!fileStream.eof()) {
		...
		if (strncmp("v ", buf, 2) == 0) { 
			...
			// process texcoord
			glm::vec2 textcoord;
			sscanf_s((buf + 3), "%f %f", &textcoord.s, &textcoord.t);
	...
	std::cout << "Loaded " << temp_uvs.size() << " UVs\n";
	std::cout << "Loaded " << temp_normals.size() << " normals\n";
	std::cout << "Processing " << vertexIndices.size() << " indices\n";
	...
	for (unsigned i = 0; i < vertexIndices.size(); ++i)
	{
		...
		{
			std::cout << "ERROR: Index out of bounds at face " << i << "\n";
			std::cout << "  Vertex: " << vertexIndex << " (max: " << temp_vertices.size() << ")\n";
			std::cout << "  UV: " << uvIndex << " (max: " << temp_uvs.size() << ")\n";
			std::cout << "  Normal: " << normalIndex << " (max: " << temp_normals.size() << ")\n";
			return false;
		}
